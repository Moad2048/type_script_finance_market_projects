...
//@version=5
strategy("A3", overlay=true, margin_long=100, margin_short=100, process_orders_on_close = true)
...
//CONDITIONS
longCondition = ((SMA1> SMA14) and (low <= S3))
if (longCondition and canTrade)
    strategy.entry("Long", strategy.long,when=longCondition)
    canTrade := false
    time >= start and end >= time
        //shortCondition
shortCondition = ((SMA1< SMA14) and (high >= R3))
...
plot(S2, title="S2", color=color.red)
