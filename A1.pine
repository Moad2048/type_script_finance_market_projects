...
//@version=5
strategy("A1", overlay=true, margin_long=100, margin_short=100, process_orders_on_close = true)
...
if (na(lastDayOfWeek) or lastDayOfWeek != currentDayOfWeek)
    canTrade := true
//timebar
start = timestamp(2022,10,19,0,0,0)
end = timestamp(2023,10,18,0,0,0)
// Get the current time
currentHour = hour
currentMinute = minute
//CONDITIONS
longCondition = ((SMA1> SMA14) and (low <= S1)) and ((currentHour <5 and currentHour >= 1) or (currentHour <10 and currentHour >= 6))
if (longCondition and canTrade)
    strategy.entry("Long", strategy.long,when=longCondition)
    canTrade := false
    time >= start and end >= time
        //shortCondition
shortCondition = ((SMA1< SMA14) and (high >= R1)) and ((currentHour <5 and currentHour >= 1) or (currentHour <10 and currentHour >= 6))
