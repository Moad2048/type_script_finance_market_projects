...
//@version=5
strategy("B3", overlay=true, margin_long=100, margin_short=100, process_orders_on_close = true)
...
//CONDITIONS
longCondition = (SMA1> SMA14) and(SMA11> SMA144) and (low<= S3) and ((currentHour <5 and currentHour >= 1) or (currentHour <10 and currentHour >= 6))
if (longCondition and canTrade)
    strategy.entry("Long", strategy.long, when= longCondition)
    canTrade := false
        //shortCondition
shortCondition = (SMA1< SMA14) and (SMA11< SMA144) and (high>= R3) and ((currentHour <5 and currentHour >= 1) or (currentHour <10 and currentHour >= 6))
...
plot(S3, title="S3", color=color.green)
